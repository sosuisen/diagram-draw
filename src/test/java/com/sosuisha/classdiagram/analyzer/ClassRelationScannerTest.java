...
class ClassRelationScannerTest {
    ...
    void scanDetectsAggregationForConstructorInjectedField() {
        ...
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().packageName().equals(FIXTURE_PKG) &&
    ...
    void scanReturnsNoRelationsForPojoWithoutSamePackageFields() {
        var relations = new ClassRelationScanner().scan(CLASS_ROOT, FIXTURE_PKG);
        assertTrue(relations.stream().noneMatch(r -> r.sourceClassInfo().packageName().equals(FIXTURE_PKG) &&
    ...
    void scanDetectsCompositionWithCollectionField() {
        var relations = new ClassRelationScanner().scan(CLASS_ROOT, FIXTURE_PKG);
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().simpleName().equals("FixtureOrder") &&
    ...
    void scanDetectsRelationFromSubpackage() {
        ...
        var relations = new ClassRelationScanner().scan(CLASS_ROOT, FIXTURE_PKG);
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().packageName().equals(subPkg) &&
    ...
    void scanDetectsRealizationForImplementsInterface() {
        var relations = new ClassRelationScanner().scan(CLASS_ROOT, FIXTURE_PKG);
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().simpleName().equals("FixtureServiceImpl") &&
    ...
    void scanDetectsBothRealizationsForMultipleInterfaceImplementation() {
        var relations = new ClassRelationScanner().scan(CLASS_ROOT, FIXTURE_PKG);
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().simpleName().equals("FixtureMultiImpl") &&
                r.targetClassInfo().simpleName().equals("FixtureService") &&
                r.type() == DependencyType.REALIZATION));
        assertTrue(relations.stream().anyMatch(r -> r.sourceClassInfo().simpleName().equals("FixtureMultiImpl") &&
